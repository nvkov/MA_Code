...
df_merge1<- df_merge1[,.(car_ID=max(car_ID), vendor_ID=paste(vendor_ID, collapse=","),
                       cars_lastChange=min(cars_lastChange), cars_lastDate=max(cars_lastDate), 
                       Anzeigenanlage=min(Anzeigenanlage), leasing=max(leasing), 
                       ...
                       group=seeOverlaps(prices_firstDate, prices_lastDate)),
