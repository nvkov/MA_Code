...
cardf<- fread("CleanReducedfahrzeugeFull_16405.txt", sep=";")
tomdf<- fread("H:\\MA\\Pkw\\generatedData\\TOM\\16408TOMMobilePreise20090118.txt", sep=";") 

setkey(cardf, "MobileID")
setkey(tomdf, "MobileID")

minimerge<- cardf[tomdf]
...
rm(minimerge, eigdf, cardf, tomdf)
...
exp<- sameCars_diffID[,.(MobileID=max(MobileID), TOM=max(TOM), Anzeigenanlage=min(Anzeigenanlage), lastDate=max(lastDate)), 
...
df<- exp[ ,.(lastDate= max(lastDate), firstDate=min(firstDate), 
