...
#survivalData<-survivalData[exchange.rate.temp,] 
...
survivalData$pln<- rep(0, nrow(survivalData))
survivalData$pln[survivalData$Bemerkung=="PLN"]<- 1
...
survivalData<- survivalData[,.(lastDate=max(Date), firstDate=min(Date), TOM=.N), by=.(MobileID, valuePrice, pln)]
