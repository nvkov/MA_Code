...
# Survival models ---------------------------------------------------------
Bmodels<- NULL
for (i in unique(factor(df1$Typ[grep("B", df1$Typ)]))){
  Bmodels[[i]]<- coxph(Surv(newTOM, status) ~ MS+ DOP + Quantile + age + 
                         Leather_seats +Full_service_history +
...
stargazer(Bmodels[["B150"]], Bmodels[["B160"]], Bmodels[["B170"]], Bmodels[["B180"]], 
          Bmodels[["B200"]], 
          apply.coef = exp,
          apply.se = exp, 
          label="tab:cphA",
          ...
          column.labels = c("B150", "B160", "B170", "B1780", "B200"), 
          summary=F, 
          align=TRUE)
sink()
